...
def main(args, config, client):
    ...
    if args.pruning:
        ...
        dataloader_for_pruning, _, _ = create_loader(datasets,samplers,batch_size=[1,1,1],
                                                     num_workers=[4,4,4],is_trains=[True,False,False], 
                                                     collate_fns=[None,None,None])
        ...
        model = blip_nlvr(client=client, pretrained='', image_size=config['image_size'], 
                            vit=config['vit'], vit_grad_ckpt=config['vit_grad_ckpt'], vit_ckpt_layer=config['vit_ckpt_layer'], 
                            evaluate=True)
        model.load_state_dict(torch.load(config['pretrained'], map_location="cpu"), strict=False)
