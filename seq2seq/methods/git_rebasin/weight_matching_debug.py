...
def weight_matching(params_list, max_iter=100, silent=False):
    # align params_list[1] to params_list[0]
    params_a = params_list[1]
    params_b = params_list[0]
    assert len(params_a) == len(params_b)
    num_layers = len(params_b)
    perm_sizes = {n: p.shape[0] for n, p in params_b}

    perm = {n: torch.arange(d) for n, d in perm_sizes.items()}

    perm_names = [n for n, p in params_b]
    ...
    for iteration in range(max_iter):
        ...
        for p_ix in layer_indices:
            ...
            for diff_indices in [-1, 1]:
                next_p_ix = p_ix + diff_indices
                if next_p_ix < 0 or next_p_ix >= num_layers:
                    continue
                next_name = perm_names[next_p_ix]
                next_permute = perm[next_name]
                ...
                if diff_indices == -1:
                    ...
                    w_b = w_b[:, next_permute]

                    A += w_a @ w_b.T
                else:
                    w_b = params_b[next_p_ix][1]
                    w_a = params_a[next_p_ix][1]
            ...
            ri, ci = torch.LongTensor(ri), torch.LongTensor(ci)
            assert (ri == torch.arange(len(ri))).all()

            oldL = torch.vdot(A.flatten(), torch.eye(d)[perm[n]].flatten())
            newL = torch.vdot(A.flatten(), torch.eye(d)[ci, :].flatten())
            if not silent: print(f"{iteration}/{n}: {newL - oldL}")
    ...
    return [[n, perm[n]] for n in perm_names]
...
def permute_based_on_block(transformer, distilled_block_ids):
    ...
    for transformer_type, num_layers, ps in zip(["encoder", "decoder"], [encoder_layers, decoder_layers], [encoder_ps, decoder_ps]):
        ...
        for block_ids_to_distill in distilled_block_ids:
            ...
            base_id = 0 # len(block_ids_to_distill) // 2 # determine which block in the list is the base
...
if __name__ == "__main__":
    ...
    permuted_model = permute_based_on_block(model, eval(config.distilled_block_ids))
