...
from seq2seq.methods.pbp_extra_input.t5_module_with_pbp import (
...
def t5_modify_with_pbp(transformer, petl_config):
    ...
    if "upsample" not in petl_config.trainable_param_names:
        if petl_config.pbp_reduction_factor == 1:
            for p_name, param in transformer.named_parameters():
                if re.fullmatch(".*downsample.*|.*upsample.*", p_name):
                    if p_name.endswith("weight"):
                        param.data = torch.eye(param.shape[0])
...
if __name__ == "__main__":
    ...
    class AdapterConfig:
        def __init__(self, adapter_type):
            ...
            self.enc_num_tokens = 1
            self.dec_num_tokens = 1
